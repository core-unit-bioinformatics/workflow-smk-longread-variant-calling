...
def expand_ont_reads(*args):
    """TODO
    this is an unnecessary c&p from above for the sole region
    to check the two assertions --- this must be refactored!
    """

    grouped_values = _group_wildcards(*args)

    assert all(s in ONT_SAMPLES for s in grouped_values["sample"])
    assert all(p in ONT_INPUT for p in grouped_values["path_id"])
